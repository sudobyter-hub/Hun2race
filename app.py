...
def query_openai(prompt):
    try:
        response = openai.Completion.create(
            engine="text-davinci-003",  # Ensure this is the correct engine name
            prompt=prompt,
            max_tokens=150,
        )
...
def generate_latex_document(
    vuln_name, vuln_desc, poc, vuln_impact, vuln_mitigation, saved_files
):
...
""" % (
    ...
    # LaTeX document body
    latex_content += r"""\section*{Vulnerability Description}
%s
...
def index():
    if request.method == "POST":
        ...
        vuln_desc = query_openai(f"Describe vulnerability {vuln_name}.")
